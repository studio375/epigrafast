...
export default async function Page({params}){
    ...
    var slides = page.acf.testimonianze.map((elem, index) => {
        return <div key={index} className="relative flex flex-col items-start justify-center gap-2 w-full h-full rounded-[20px] bg-[#F1F3F5] pt-15 pb-18 max-m:pt-10 max-m:pb-10 max-s:pt-5 max-s:pb-5 px-16 max-l:px-10 max-m:px-5 max-s:px-2">
            ...
            <span className="w-full block text-end font-bold uppercase m:absolute m:bottom-4 m:ight-7">{elem.autore}</span>
        </div>
    })
    return <>
        ...
        <ScrollGallery className="boxed xl:!px-32 mb-10" slides={slides} />
    </>   
}   