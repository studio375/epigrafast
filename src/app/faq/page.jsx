...
export default async function Page({params}){
    ...
    return <>
        ...
        <section className="relative w-full boxed flex flex-wrap gap-2 items-stretch justify-start mb-12 max-s:mb-10">
            {
                faq.map((elem, index) => {
                    return <div key={index} className="w-[calc(50%-10px)] max-s:w-full flex flex-col items-start justify-center px-[115px] max-l:px-8 max-m:px-5 max-s:px-2 bg-[#F1F3F5] m:min-h-40 max-m:py-6 max-s:py-4 rounded-[20px] gap-[15px]">
                        <span className="font-bold uppercase block">{elem.domanda}</span>
                        <span className="h2 block">{elem.risposta}</span>
                    </div>
                })
            }
            <div className="w-full flex flex-col items-center justify-center gap-6 max-s:gap-3 px-[115px] max-l:px-8 max-m:px-5 bg-[#F1F3F5] m:min-h-40 max-m:py-6 max-s:py-4 rounded-[20px]">
