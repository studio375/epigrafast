...
import VenetoMap from "@/components/Library/Home/venetoMap";
...
export default async function Home() {
  ...
  return <>
    ...
    <section className="relative mt-5 boxed flex flex-col items-center gap-9 max-m:gap-7 max-s:gap-5 max-s:-mt-5 max-xxs:-mt-12">
      <Paragraph className="text-center w-120 max-w-full">{page.acf.paragrafo}</Paragraph>
    ...
    <section className="relative w-full pt-13 max-s:pt-7 boxed flex flex-col items-center gap-9">
        ...
        <div className="flex flex-col items-start flex-1 relative gap-[15px]">
          {
            page.acf.obiettivi.map((elem, index) => {
              return <div key={index} className="relative flex flex-col gap-[15px] [&:last-child_.line]:hidden">
                <div className="flex items-center w-full gap-3 show-on-scroll">
                ...
                <div className="line relative w-[1px] h-7 bg-[var(--primary)] ml-[47.5px] max-xs:ml-[25px]">
                  <div className="line-animation h-full w-full absolute left-0 bottom-0 bg-white"></div>
                </div>                
              </div>
            })
          }
      ...
      <div className="flex items-center w-full l:pr-12 gap-18 max-l:gap-10 max-s:gap-5 -mt-10 max-l:mt-0 max-s:flex-col-reverse">
        <VenetoMap className="flex-1 relative" imgObject={page.acf.immagine_dove_operiamo} />
